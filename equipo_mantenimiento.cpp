...
void equipo_mantenimiento::actualizar_operario() {
    ...
    do{
        ...
        cin >> aux;
    } while (aux>integrantes or aux<1); //Ciclo condicionado para asegurar que el valor indicado está dentro del rango de integrantes
    cin>>equipo[aux-1]; //Se solicita la entrada de los nuevos datos.
}

//sobrecarga de salida
ostream &operator << (ostream &cout, const equipo_mantenimiento &a){
    for (int i=0; i<a.equipo.size();i++){
        cout << " -------------- Integrante N°" << i+1 << " -------------- " <<endl;
        cout << a.equipo[i] << endl;
    }
    return cout;
}